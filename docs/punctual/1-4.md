...
**Goals:**
 ...
 - use textures (images) in your creations
...
a + b >> video;
```
Nice and simple! Plus we can reuse the variables as many times as we like - let's reuse it with `min` and add some oscillators for fun
```
a << (circle [0,0] 0.25);
b << (circle [0.15,0.15] 0.35);
a + b >> video;
min (sin a) (tri b) >> video;
```
...
**XXX: todo `img` now `tex*` is deprecated**
