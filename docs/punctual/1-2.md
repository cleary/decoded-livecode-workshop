...
Let's take our simple bouncing circle example, and set boundaries on the minimum and maximum values so the centre of the screen is no longer the low point - let's make it move from the bottom of the screen `-1`, to the top of the screen `1`:

```
circle [0, (-1) ~~ 1 $ sin 1] 0.02 >> video;
```
