...
This is an example of a sine `osc`illator, notice how the ball in the middle follows the sine wave shape over time. The sine wave has a specific characteristic that it tends to accelerate towards the middle, then decelerate after it passes the mid-point
...
2. Describe the difference you see between `osc` and `tri` (hint, keep the oscillator value 1 or below)
